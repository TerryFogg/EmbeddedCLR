...
#define NULL_I2C_MEM_ADDR                                                                                              \
    0xFF /*!< set mem_address to NULL_I2C_MEM_ADDR if i2c device has no internal address during read/write */
#define NULL_I2C_MEM_16BIT_ADDR                                                                                        \
    0XFFFF /*!< set 16bit mem_address to NULL_I2C_MEM_16BIT_ADDR if i2c device has no internal address during          \
              read/write */
...
#define portTICK_RATE_MS     portTICK_PERIOD_MS
    ...
    /**
     * @brief Create an I2C bus instance then return a handle if created successfully. Each I2C bus works in a singleton
     * mode, which means for an i2c port only one group parameter works. When i2c_bus_create is called more than one
     * time for the same i2c port, following parameter will override the previous one.
     *
     * @param port I2C port number. Please note that enabling I2C_BUS_SUPPORT_SOFTWARE in menuconfig allows you to use
     * ports in i2c_sw_port_t to enable software I2C.
     ...
     * @param clk_speed device specified clock frequency the i2c_bus will switch to during each transfer. 0 if use
     * current bus speed.
    ...
    /**
     * @brief Delete and release the I2C device resource, i2c_bus_device_delete should be used in pairs with
     ...
     *        If I2C_BUS_DYNAMIC_CONFIG enable, i2c_bus will dynamically check configs and re-install i2c driver before
     * each transfer, hence multiple devices with different configs on a single bus can be supported.
     *        @note
     ...
     *
     * @param dev_handle I2C device handle
     * @param mem_address The internal 16-bit reg/mem address to write to, set to NULL_I2C_MEM_ADDR if no internal
     * address.
     * @param data_len Number of bytes to write
     * @param data Pointer to the bytes to write.
     * @return esp_err_t
     ...
     *
     * @param dev_handle I2C device handle
     * @param mem_address The internal 16-bit reg/mem address to read from, set to NULL_I2C_MEM_ADDR if no internal
     * address.
     * @param data_len Number of bytes to read
     * @param data Pointer to a buffer to save the data that was read
     * @return esp_err_t
